...
export function KineticTextLoader({
  ...
  return (
    <div
      className={cn(
        "relative flex items-center justify-center font-light",
        className
      )}
      {...props}
    >
      ...
      <div className="relative scale-75 md:scale-90 lg:scale-100">
        <div
          className="absolute z-10 top-[40px] left-[85px] h-[6px] w-[6px] rounded-full bg-neutral-800 dark:bg-neutral-200"
          style={{
            animation:
              "ktl-dotMove 1800ms cubic-bezier(0.25,0.25,0.75,0.75) infinite",
          }}
        />
          ...
          {letters.map((char, index) => {
            if (index === 0 && char.toUpperCase() === "L") {
              return (
                <span
                  key={index}
                  className="relative inline-block origin-[100%_70%] tracking-[8px]"
                  style={{
                    animation:
                      "ktl-l-bounce 1800ms cubic-bezier(0.25,0.25,0.75,0.75) infinite",
                  }}
                >
                  {char}
                </span>
